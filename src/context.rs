...
/// Check if a code entity is noise (tests, imports, trivial helpers).
fn is_noise(name: &str, node_type: &str, definition: &str) -> bool {
    // Always filter: test functions, imports, use statements
    if node_type == "Import" {
        return true;
    }
    ...
    if node_type == "Function" && definition.starts_with("fn test_") {
        return true;
    }
    // Filter generic/duplicate names
    if name == "default" || name == "new" || name == "fmt" {
        return true;
    }
    // Filter field details for code entities — too granular
    if node_type == "Field" {
        return true;
    }
    false
...
/// Sanitize output for hook safety — no characters that could break JSON/shell parsing.
fn sanitize_for_hook(s: &str) -> String {
    s.replace('\\', "/")
        .replace('"', "'")
        .replace('\t', " ")
        .lines()
...
/// Filters noise (tests, imports, fields) and sanitizes output for hook safety.
pub fn generate_context(kg: &KnowledgeGraph, budget: usize, now: u64) -> String {
    let char_budget = budget * 4;

    // Collect all nodes with relevance > 0, filtering noise
    ...
    for node in kg.all_nodes() {
        ...
        // Filter noise for code entities
        if is_code && is_noise(&node.name, &node.node_type, &node.definition) {
            continue;
        }
    ...
    let mut output = String::from("[KG Context]\n");
    ...
    for (_rel, tier, name, definition, node_type) in &scored {
        ...
        let clean_def = sanitize_for_hook(definition);
        let line = format!("- {} ({}): {}\n", name, node_type, clean_def);
...
mod tests {
    ...
    fn test_context_empty_kg() {
        ...
        let output = generate_context(&kg, 2000, 100 * 86400);
        assert!(output.contains("[KG Context]"));
        assert!(!output.contains("[Critical]"));
    }
}
